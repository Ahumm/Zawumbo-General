...
(define (myeval x binds)
  (cond ((symbol? x) ;; Return value if set using mylet, else return the symbol
            (if (equal? (lookup x binds) #f)
               x
               (lookup x binds)))
        ((integer? x) x) ;; Read a number
...
(define (evaltoks x binds)
   (cond ((equal? (myeval (car x) binds) 'prog) (myeval (car (cdr x)) binds)) ;; Top of program, recurse
         ((equal? (myeval (car x) binds) 'myignore) 0) ;; Return 0 on myignore
         ((equal? (myeval (car x) binds) 'myadd) ;; Add first two expressions in x recursively on myadd
             (+ (myeval (car (cdr x)) binds) (myeval (car (cdr (cdr x))) binds)))
         ...
         ((equal? (myeval (car x) binds) 'mylet) ;; handle adding new variables to the binds list, messy due to handling scope issues
             (myeval (car (cdr (cdr (cdr x)))) (cons (list (car (cdr x)) (myeval (car (cdr (cdr x))) binds)) binds)))))
